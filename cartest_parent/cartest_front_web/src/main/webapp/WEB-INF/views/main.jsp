<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<!DOCTYPE html>
<html lang="zh">
<head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0"/>
    <meta http-equiv="X-UA-Compatible" content="ie=edge" />
    <title>机动车理论考试答题系统</title>
    <link rel="stylesheet"  href="${pageContext.request.contextPath}/css/bootstrap.css" />
    <link rel="stylesheet"  href="${pageContext.request.contextPath}/css/index.css" />
    <script src="${pageContext.request.contextPath}/js/jquery.js"></script>
    <script src="${pageContext.request.contextPath}/js/bootstrap.js"></script>
    <script src="${pageContext.request.contextPath}/js/userSetting.js"></script>
    <script type="text/javascript">
        $(function(){
            // 点击切换页面
            $("#product-type-set").click(function() {
                $("#frame-id").attr("src", "${pageContext.request.contextPath}/student");
            });
            $("#product-set").click(function() {
                $("#frame-id").attr("src", "${pageContext.request.contextPath}/question/studentPin");
            });
            $("#user-set").click(function() {
                $("#frame-id").attr("src", "customerManager.html");
            });
            $("#manager-set").click(function() {
                $("#frame-id").attr("src", "sysuserManager.html");
            });
        });
        $(function(){
            $(".user-setting li").click(function(){
                $(".user-setting li").removeClass("active");
                $(this).addClass("active");
                var panelId = "#"+$(this).attr("name");
                $("#userPanel > div").css({'display':'none'});
                $(panelId).css({'display':'block'});

            });

            $("#exit").on("click",function () {
                $.ajax({
                    type: "POST",
                    url: '${pageContext.request.contextPath}/user/exit',
                    contentType: "application/json;charset=UTF-8",//必须
                    dataType: "json",//必须
                    success: function(result){
                        if(result.status == 1){
                            location.href = '${pageContext.request.contextPath}/showlogin';
                        }
                    }
                })
            })
        });
    </script>
</head>

<body>
<div class="wrapper-cc clearfix">
    <div class="content-cc">
        <!-- header start -->
        <div class="clear-bottom head">
            <div class="container head-cc">
                <p>机动车理论考试<span>答题系统</span></p>
                <div class="welcome">
                    <div class="left" style="padding-right: 30px">欢迎您：</div>
                    <div class="right">${sessionScope.user.getUser_name()}</div>
                    <input type="button" value="退出" class="btn btn-primary" id="exit">
                </div>
            </div>
        </div>
        <!-- header end -->
        <!-- content start -->
        <div class="container-flude flude-cc" id="a">
            <div class="row user-setting">
                <div class="col-xs-2 user-wrap">
                    <ul class="list-group">
                        <li class="list-group-item active" name="userSet" id="product-type-set">
                            <i class="glyphicon glyphicon-lock"></i> &nbsp;考生注册
                        </li>
                        <li class="list-group-item" name="userPic" id="product-set">
                            <i class="glyphicon glyphicon-facetime-video"></i> &nbsp;考试答题
                        </li>
                        <%--<li class="list-group-item" name="userInfo" id="user-set">--%>
                            <%--<i class="glyphicon glyphicon-user"></i> &nbsp;试题回放--%>
                        <%--</li>--%>
                        <%--<li class="list-group-item" name="adminSet" id="manager-set">--%>
                            <%--<i class="glyphicon glyphicon-globe"></i> &nbsp;用户管理--%>
                        <%--</li>--%>
                    </ul>
                </div>
                <div class="col-xs-10" id="userPanel">
                    <iframe id="frame-id" src="${pageContext.request.contextPath}/student" width="100%" height="100%" frameborder="0" scrolling="no">
                    </iframe>
                </div>

            </div>
        </div>
        <!-- content end-->
    </div>
</div>
<!-- footers start -->
<div class="footer">
    @机动车理论考试答题系统
</div>
<!-- footers end -->
</body>

</html>
